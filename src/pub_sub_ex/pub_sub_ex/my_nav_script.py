...
class NavGoalSender(Node):
    ...
    def send_goal(self, x, y, theta, position_tol=0.20, angle_tol=0.35):
        ...
        check_rate = self.create_rate(5)  # Check 5 times per second
        ...
        while True:
            ...
            if self.goal_handle.status in [
                action_msgs.msg.GoalStatus.STATUS_ABORTED,
                action_msgs.msg.GoalStatus.STATUS_CANCELED
            ]:
                # If we canceled it ourselves because we're close, that's OK
                if self.is_near_goal(x, y, theta, position_tol * 1.5, angle_tol * 1.5):
                    ...
                    self.get_logger().error(f'Navigation failed with status: {self.goal_handle.status}')
                    return False
            
            # Check if succeeded normally
            if self.goal_handle.status == action_msgs.msg.GoalStatus.STATUS_SUCCEEDED:
